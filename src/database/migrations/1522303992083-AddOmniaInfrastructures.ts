...
export class AddOmniaInfrastructures1522303992083 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("CREATE TABLE `entry` (`key` varchar(1024) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, `data` json NOT NULL, `parentKey` varchar(1024) CHARACTER SET ascii COLLATE ascii_bin, PRIMARY KEY(`key`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `property` (`key` varchar(1024) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, `isObsolete` tinyint(4) NOT NULL, `name` varchar(128) NOT NULL, `description` text NOT NULL, `type` enum('String', 'Identifier', 'Markdown', 'Integer', 'Decimal', 'Reference', 'Year', 'Date') NOT NULL, `metadata` json NOT NULL, `comparability` enum('NotComparable', 'PlainComparable', 'BiggerBetter', 'SmallerBetter') NOT NULL DEFAULT 'NotComparable', `aspectKey` varchar(1024) CHARACTER SET ascii COLLATE ascii_bin, PRIMARY KEY(`key`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `aspect` (`key` varchar(1024) CHARACTER SET ascii COLLATE ascii_bin NOT NULL, `isObsolete` tinyint(4) NOT NULL, `name` varchar(128) NOT NULL, `description` text NOT NULL, PRIMARY KEY(`key`)) ENGINE=InnoDB");
